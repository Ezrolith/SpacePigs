...
- **Permanent upgrades shop** (start screen): Extra Life (+1 starting life per level, 250 credits each, stackable 3x) and Attack Speed (+10% fire rate per level, 250 credits each, stackable 3x for 30%). Stored in Firestore `users` doc `upgrades` field, persist forever. Mushroom pickup cap still uses `ds.maxLives`.
